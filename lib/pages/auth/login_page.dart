...
import 'package:task1/analytics/actions.dart';
...
class LoginPage extends StatefulWidget {
  ...
  const LoginPage({super.key, this.initialLoginMethod});
...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> _loginWithEmail() async {
    ...
    try {
      ...
      await _checkFirstLogin(); // Check if it's the user's first login
      

      Navigator.of(context).pushReplacement(
        MaterialPageRoute(builder: (_) => const MainNavigationPage()), // Replace current page with MainNavigationPage
      );
    ...
    try {
      await FirebaseAuth.instance.verifyPhoneNumber(
        ...
        verificationCompleted: (PhoneAuthCredential credential) async {
          ...
          await _checkFirstLogin(); // Check if it's the user's first login
           _showSuccessSnackBar('Phone Sign-In Successful');
        },
    ...
    try {
      ...
      if (user != null) {
        ...
        if (userSnapshot.exists) {
          ...
          bool isFirstLogin = userData?['firstLogin'] ?? false;
          if (isFirstLogin) {
           // Show the first login experience (e.g., tutorial or welcome message)
            // _showSuccessSnackBar('Welcome! This is your first login.');
            userActions.logFirstTimeLogin(userId);
            _showSuccessSnackBar('first sign in welcome');

            // After first login, set firstLogin to false
            await userRef.update({'firstLogin': false});
          } else {
            // Regular login
            _showSuccessSnackBar('Welcome back!');
          }
          // First login logic - Store user data in Firestore using UID as the document ID
          

          // Show welcome notification for first-time users
        } else {
          // User does not exist in Firestore, handle error
          _showErrorSnackBar('User does not exist in Firestore.');
        }
      }
    } catch (e) {
      _showErrorSnackBar('Failed to check first login: $e');
    }
  }
